...
extern "C" {
__global__ void __kernel__compute_weights_fwd(TensorView sigmas_0, TensorView steps_0, TensorView info_0, float threshold_0, TensorView weights_0, int n_rays_0)
{
    int idx_0 = int(((threadIdx)).x + ((blockDim)).x * ((blockIdx)).x);
    if(idx_0 >= n_rays_0)
    {
        return;
    }
    int _S1 = int(2) * idx_0;
    int _S2 = ((info_0).load<int>((uint(_S1 + int(1)))));
    int _S3 = ((info_0).load<int>((uint(_S1))));
    int _S4 = _S3 + _S2;
    if(_S2 == int(0))
    {
        return;
    ...
    for(;;)
    {
        if(transmittance_0 > threshold_0 && k_0 < _S4)
        {
        }
        else
        {
        ...
        uint _S5 = uint(k_0);
        float _S6 = ((sigmas_0).load<float>((_S5)));
        float _S7 = - _S6;
        float _S8 = ((steps_0).load<float>((_S5)));
        float alpha_0 = (F32_exp((_S7 * _S8)));
        (weights_0).store<float>((_S5), (transmittance_0 * (1.0f - alpha_0)));
        int _S9 = k_0 + int(1);
        int _S10 = loops_0 + int(1);
        transmittance_0 = transmittance_0 * alpha_0;
        k_0 = _S9;
        loops_0 = _S10;
    }
    printf("%i", loops_0);
...
extern "C" {
__global__ void __kernel__compute_weights_bwd(TensorView sigmas_1, TensorView steps_1, TensorView info_1, TensorView weights_1, TensorView grad_weights_0, TensorView grad_sigmas_0, int n_rays_1)
{
    int idx_1 = int(((threadIdx)).x + ((blockDim)).x * ((blockIdx)).x);
    if(idx_1 >= n_rays_1)
    {
        return;
    }
    int _S11 = int(2) * idx_1;
    int _S12 = ((info_1).load<int>((uint(_S11 + int(1)))));
    int _S13 = ((info_1).load<int>((uint(_S11))));
    int _S14 = _S13 + _S12;
    if(_S12 == int(0))
    {
        return;
    ...
    if(_S15)
    {
        ...
        {
            uint _S17 = uint(k_1);
            float _S18 = ((weights_1).load<float>((_S17)));
            float _S19 = ((grad_weights_0).load<float>((_S17)));
            float acc_1 = acc_0 - _S18 * _S19;
            _S16 = acc_1;
            int k_2 = k_1 + int(1);
            if(k_2 < _S14)
            {
    ...
    {
        k_1 = _S13;
        float transmittance_1 = 1.0f;
        for(;;)
        {
            uint _S20 = uint(k_1);
            float _S21 = ((weights_1).load<float>((_S20)));
            float _S22 = ((grad_weights_0).load<float>((_S20)));
            float acc_2 = acc_0 + _S21 * _S22;
            float _S23 = ((sigmas_1).load<float>((_S20)));
            float _S24 = - _S23;
            float _S25 = ((steps_1).load<float>((_S20)));
            float transmittance_2 = transmittance_1 * (F32_exp((_S24 * _S25)));
            float _S26 = ((steps_1).load<float>((_S20)));
            float _S27 = ((grad_weights_0).load<float>((_S20)));
            (grad_sigmas_0).store<float>((_S20), (_S26 * (acc_2 + transmittance_2 * _S27)));
            int k_3 = k_1 + int(1);
            if(k_3 < _S14)
            {
            }
